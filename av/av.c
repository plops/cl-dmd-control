...
int vid_get_thread_count(Vid*v){return v->avctx->thread_count;}
int vid_get_thread_type(Vid*v){return v->avctx->thread_type;}
...
int vid_init(Vid*v,const char*fn,int ow,int oh)
{
  ...
  //v->avctx->thread_count=4;
  //AVDictionary *opts;
  v->avctx->flags2 |= CODEC_FLAG2_FAST;
  v->avctx->thread_count = 4;
  //if (!av_dict_get(opts, "threads", NULL, 0))
  //  av_dict_set(&opts, "threads", "auto", 0);

  if(avcodec_open2(v->avctx,v->codec,NULL /*&opts*/)<0){
    pthread_mutex_unlock(&m);
    return -5;
  }
